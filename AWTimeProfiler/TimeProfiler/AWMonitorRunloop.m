...
@implementation AWMonitorRunloop

+ (instancetype)sharedInstance {
    static dispatch_once_t once;
    static AWMonitorRunloop *sharedInstance;
    dispatch_once(&once, ^{
...
static void runLoopObserverCallBack(CFRunLoopObserverRef observer, CFRunLoopActivity activity, void *info) {
    AWMonitorRunloop *instance = [AWMonitorRunloop sharedInstance];
    // 记录状态值
        ...
        while (YES) {
            ...
            if (dsw != 0) {
                if (self->_activity == kCFRunLoopBeforeSources || self->_activity == kCFRunLoopAfterWaiting) {
                    ...
                    NSString *backtrace = [AWCallStack aw_backtraceOfMainThread];
                    NSLog(@"++++%@",backtrace);
